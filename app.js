...
{
	if (region.T.H === region._.H)
	{
		return 'on line ' + region.T.H;
	}
	return 'on lines ' + region.T.H + ' through ' + region._.H;
}
...
var $author$project$CreditInterestAllocationEditor$init = function (flags) {
	...
	return _Utils_Tuple2(
		{M: flags.M, v: fee_plans, G: false, N: fee_plans},
	...
	function (model, installments_count, maybe_value) {
		...
		var maybe_customer_fee_variable = A2(
			...
			function (value) {
				...
				return (!fee_plan.aT) ? ((!(!original_fee_plan.aT)) ? $elm$core$Maybe$Just(original_fee_plan.aT) : $elm$core$Maybe$Just(1)) : (_Utils_eq(value, -1) ? $elm$core$Maybe$Just(-1) : ((!value) ? $elm$core$Maybe$Just(0) : $elm$core$Maybe$Just(cappedValue)));
			},
			maybe_value);
			...
			default:
				if (msg.b.$ === 1) {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{v: new_fee_plans, G: false, N: new_original_fee_plans}),
						$elm$core$Platform$Cmd$none);
				}
		}
	});
	...
	function (options, currentValue) {
		...
		var filterKey = function (event) {
			return (event.aR || (event.aI || event.a7)) ? _Utils_Tuple2(
				...
				false) : (A2(
				$elm$core$List$any,
				$elm$core$Basics$eq(event.a0),
				$author$project$Input$KeyCode$allowedKeyCodes) ? _Utils_Tuple2(
				options.c(currentValue),
				false) : (((isNumber(event.a0) || isNumPad(event.a0)) && A2(
...
var $author$project$Views$FeePlan$showInterestPanel = function (_v0) {
	...
	var effectiveClientFee = A2($elm$core$Basics$min, customer_fee_variable, maximum_interest_rate.aL);
					...
					[
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										(_Utils_cmp(maximum_interest_rate.aL, totalFees) < 0) ? A2(
										$elm$html$Html$div,
										_List_fromArray(
											[
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$elm$html$Html$text(
										$author$project$Views$Utils$percents(maximum_interest_rate.aL))
									])),
										...
										[
											...
											_List_fromArray(
												[
													...
													$elm$html$Html$Attributes$disabled(is_sending)
												]),
											_List_fromArray(
												[
...
var $author$project$CreditInterestAllocationEditor$view = function (_v0) {
	...
	var is_sending = _v0.G;
	return A2(
		$elm$html$Html$div,
		_List_Nil,
		A2(
			$elm$core$List$map,
			$author$project$Views$FeePlan$show(is_sending),
